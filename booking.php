...
if(isset($_GET['date'])){
  ...
  $bookings = get_bookings($date, $time);
    ...
    <div>
      <form class='container'  method='post' action='booking.php'>
        ...
        <div  class='booking'>
          ...
          <select id='time' class='input-field'  name='time'  onchange='checkAvailability(event);' required >       
      ...
      $.ajax({
        ...
        success: (data) => {
          ...
          facilityOpts.forEach(opt => {
            document.querySelector("#facility").insertAdjacentElement('beforeend', opt); count++;
          });
          if (!count)
            document.querySelector("#facility").insertAdjacentHTML('beforeend', "<option disabled selected>No facility available</option>");
